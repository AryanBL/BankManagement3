/* =========================================================
   SampleData_RealisticScenario_RESILIENT.sql
   ---------------------------------------------------------
   Realistic sample-data scenario for the final BankManagement
   database project. This resilient version catches non-critical
   demo-scenario failures and logs them instead of inserting NULL IDs.
...
IF OBJECT_ID('tempdb..#SampleIssues') IS NOT NULL DROP TABLE #SampleIssues;
...
BEGIN
    ...
    BEGIN TRY
        IF @AUserID IS NULL OR @ACustomerID IS NULL OR @ABranchID IS NULL OR @ATypeID IS NULL
        BEGIN
            RAISERROR('Missing lookup value for account plan row.', 16, 1);
        END;
    ...
    BEGIN CATCH
        ...
        INSERT INTO #SampleIssues(StepName, ItemKey, ErrorNumber, ErrorSeverity, ErrorState, ErrorLine, ErrorProcedure, ErrorMessage)
        VALUES(N'AccountOpen', @AccountKey, ERROR_NUMBER(), ERROR_SEVERITY(), ERROR_STATE(), ERROR_LINE(), ERROR_PROCEDURE(), ERROR_MESSAGE());
        PRINT CONCAT(N'WARNING: account plan skipped: ', @AccountKey, N' - ', ERROR_MESSAGE());
    END CATCH;

    IF @AccountID IS NOT NULL AND @AccountNumber IS NOT NULL
    BEGIN
    ...
    END
    ELSE IF NOT EXISTS (SELECT 1 FROM #SampleIssues WHERE StepName = N'AccountOpen' AND ItemKey = @AccountKey)
    BEGIN
        INSERT INTO #SampleIssues(StepName, ItemKey, ErrorMessage)
        VALUES(N'AccountOpen', @AccountKey, N'sp_Account_Open completed but did not return AccountID/AccountNumber. Row skipped.');
        PRINT CONCAT(N'WARNING: account plan skipped because no AccountID was returned: ', @AccountKey);
    END;
...
BEGIN CATCH
    IF XACT_STATE() <> 0 ROLLBACK TRANSACTION;
    INSERT INTO #SampleIssues(StepName, ItemKey, ErrorNumber, ErrorSeverity, ErrorState, ErrorLine, ErrorProcedure, ErrorMessage)
    VALUES(N'InitialDepositBatch', NULL, ERROR_NUMBER(), ERROR_SEVERITY(), ERROR_STATE(), ERROR_LINE(), ERROR_PROCEDURE(), ERROR_MESSAGE());
    PRINT CONCAT(N'WARNING: initial-deposit batch processing failed - ', ERROR_MESSAGE());
END CATCH;
...
INSERT INTO #TransactionPlan VALUES
    ...
    (N'TX22', N'Transfer', N'CUST_NOAH', N'A_NOAH_BUS', N'A_SOPHIA_BUS', 28000, N'Large pending supplier transfer', N'LeavePending'),
...
DECLARE
    ...
    @OperationsUserID INT, @TxExecutionSucceeded BIT;
...
BEGIN
    ...
    SET @TxExecutionSucceeded = 0;

    BEGIN TRY
        IF @ActorUserID IS NULL
            RAISERROR('Transaction actor user was not found in #AppUsers.', 16, 1);
        IF @ActionName IN (N'Withdrawal', N'Transfer') AND @FromAccountID IS NULL
            RAISERROR('Source account was not found in #Accounts.', 16, 1);
        IF @ActionName IN (N'Deposit', N'Transfer') AND @ToAccountID IS NULL
            RAISERROR('Destination account was not found in #Accounts.', 16, 1);
        ...
        ELSE
        BEGIN
            RAISERROR('Unknown transaction ActionName.', 16, 1);
        END;

        SET @TxExecutionSucceeded = 1;
    END TRY
    BEGIN CATCH
        IF XACT_STATE() <> 0 ROLLBACK TRANSACTION;
        INSERT INTO #SampleIssues(StepName, ItemKey, ErrorNumber, ErrorSeverity, ErrorState, ErrorLine, ErrorProcedure, ErrorMessage)
        VALUES(CONCAT(N'TransactionCreate-', @ActionName), @TxKey, ERROR_NUMBER(), ERROR_SEVERITY(), ERROR_STATE(), ERROR_LINE(), ERROR_PROCEDURE(), ERROR_MESSAGE());
        PRINT CONCAT(N'WARNING: transaction scenario skipped during creation: ', @TxKey, N' - ', ERROR_MESSAGE());
    END CATCH;
    ...
    BEGIN
        ...
        BEGIN CATCH
            IF XACT_STATE() <> 0 ROLLBACK TRANSACTION;
            INSERT INTO #SampleIssues(StepName, ItemKey, ErrorNumber, ErrorSeverity, ErrorState, ErrorLine, ErrorProcedure, ErrorMessage)
            VALUES(CONCAT(N'TransactionOutcome-', @Outcome), @TxKey, ERROR_NUMBER(), ERROR_SEVERITY(), ERROR_STATE(), ERROR_LINE(), ERROR_PROCEDURE(), ERROR_MESSAGE());
            PRINT CONCAT(N'WARNING: transaction scenario outcome failed: ', @TxKey, N' - ', ERROR_MESSAGE());
        END CATCH;
    END
    ELSE IF @TxExecutionSucceeded = 1
    BEGIN
        INSERT INTO #SampleIssues(StepName, ItemKey, ErrorMessage)
        VALUES(CONCAT(N'TransactionCreate-', @ActionName), @TxKey, N'Transaction procedure completed but did not return TransactionID. Outcome step skipped.');
        PRINT CONCAT(N'WARNING: transaction procedure returned no TransactionID. Scenario skipped: ', @TxKey);
    END;
...
BEGIN TRY
    ...
    IF @DormantAccount1 IS NOT NULL AND @IsabellaUserID IS NOT NULL
    BEGIN
        ...
        IF @ReactivationTxID IS NOT NULL
        BEGIN
    ...
    IF @FreezeAccountID IS NOT NULL
...
BEGIN CATCH
    IF XACT_STATE() <> 0 ROLLBACK TRANSACTION;
    INSERT INTO #SampleIssues(StepName, ItemKey, ErrorNumber, ErrorSeverity, ErrorState, ErrorLine, ErrorProcedure, ErrorMessage)
    VALUES(N'AccountStatusCases', NULL, ERROR_NUMBER(), ERROR_SEVERITY(), ERROR_STATE(), ERROR_LINE(), ERROR_PROCEDURE(), ERROR_MESSAGE());
    PRINT CONCAT(N'WARNING: account status demo cases partially skipped - ', ERROR_MESSAGE());
END CATCH;
...
BEGIN
    ...
    BEGIN TRY
        IF @LoanCustomerID IS NULL OR @LoanBranchID IS NULL OR @CreatorUserID IS NULL
            RAISERROR('Missing lookup value for loan plan row.', 16, 1);
    ...
    BEGIN CATCH
        IF XACT_STATE() <> 0 ROLLBACK TRANSACTION;
        INSERT INTO #SampleIssues(StepName, ItemKey, ErrorNumber, ErrorSeverity, ErrorState, ErrorLine, ErrorProcedure, ErrorMessage)
        VALUES(N'LoanCreate', @LoanKey, ERROR_NUMBER(), ERROR_SEVERITY(), ERROR_STATE(), ERROR_LINE(), ERROR_PROCEDURE(), ERROR_MESSAGE());
        PRINT CONCAT(N'WARNING: loan plan skipped during creation: ', @LoanKey, N' - ', ERROR_MESSAGE());
    END CATCH;
    ...
    BEGIN
        ...
        BEGIN TRY
            ...
            BEGIN
                ...
                IF @InstallmentID IS NULL OR @PayAccountID IS NULL OR @CreatorUserID IS NULL
                    RAISERROR('Missing installment, payment account, or customer user for loan payment scenario.', 16, 1);

                SET @PayTxID = NULL; SET @PayReady = NULL;
                ...
                BEGIN
                    SET @InstallmentID = NULL; SET @PayTxID = NULL; SET @PayReady = NULL;
                    SELECT TOP(1) @InstallmentID = InstallmentID FROM dbo.Installment WHERE LoanID = @LoanID AND InstallmentStatus = N'Pending' ORDER BY DueDate;
                    IF @InstallmentID IS NULL
                        RAISERROR('Second installment was not found for PayTwo scenario.', 16, 1);
                    EXEC dbo.sp_Loan_PayInstallment @InstallmentID = @InstallmentID, @FromAccountID = @PayAccountID, @EmployeeID = NULL, @TransactionID = @PayTxID OUTPUT, @ReadyToCompleteAt = @PayReady OUTPUT, @UserID = @CreatorUserID;
                    IF @PayTxID IS NOT NULL
                    BEGIN
                        UPDATE dbo.Transactions SET ReadyToCompleteAt = DATEADD(MINUTE, -10, GETDATE()) WHERE TransactionID = @PayTxID;
        ...
        BEGIN CATCH
            IF XACT_STATE() <> 0 ROLLBACK TRANSACTION;
            INSERT INTO #SampleIssues(StepName, ItemKey, ErrorNumber, ErrorSeverity, ErrorState, ErrorLine, ErrorProcedure, ErrorMessage)
            VALUES(N'LoanScenario', @LoanKey, ERROR_NUMBER(), ERROR_SEVERITY(), ERROR_STATE(), ERROR_LINE(), ERROR_PROCEDURE(), ERROR_MESSAGE());
            PRINT CONCAT(N'WARNING: loan scenario partially skipped: ', @LoanKey, N' - ', ERROR_MESSAGE());
        END CATCH;
    END
    ELSE IF NOT EXISTS (SELECT 1 FROM #SampleIssues WHERE StepName = N'LoanCreate' AND ItemKey = @LoanKey)
    BEGIN
        INSERT INTO #SampleIssues(StepName, ItemKey, ErrorMessage)
        VALUES(N'LoanCreate', @LoanKey, N'sp_Loan_Create completed but did not return LoanID. Loan scenario skipped.');
        PRINT CONCAT(N'WARNING: loan procedure returned no LoanID. Scenario skipped: ', @LoanKey);
    END;
...
BEGIN CATCH
    IF XACT_STATE() <> 0 ROLLBACK TRANSACTION;
    INSERT INTO #SampleIssues(StepName, ItemKey, ErrorNumber, ErrorSeverity, ErrorState, ErrorLine, ErrorProcedure, ErrorMessage)
    VALUES(N'LoanOverdueProcessor', NULL, ERROR_NUMBER(), ERROR_SEVERITY(), ERROR_STATE(), ERROR_LINE(), ERROR_PROCEDURE(), ERROR_MESSAGE());
    PRINT CONCAT(N'WARNING: loan overdue processor failed - ', ERROR_MESSAGE());
END CATCH;
...
BEGIN CATCH
    IF XACT_STATE() <> 0 ROLLBACK TRANSACTION;
    INSERT INTO #SampleIssues(StepName, ItemKey, ErrorNumber, ErrorSeverity, ErrorState, ErrorLine, ErrorProcedure, ErrorMessage)
    VALUES(N'MonthlyInterest', NULL, ERROR_NUMBER(), ERROR_SEVERITY(), ERROR_STATE(), ERROR_LINE(), ERROR_PROCEDURE(), ERROR_MESSAGE());
    PRINT CONCAT(N'WARNING: monthly interest sample step failed - ', ERROR_MESSAGE());
END CATCH;
...
IF EXISTS (SELECT 1 FROM #SampleIssues)
BEGIN
    PRINT 'Sample data completed with warnings. Review the rows below.';
    SELECT IssueID, StepName, ItemKey, ErrorNumber, ErrorProcedure, ErrorLine, ErrorMessage, CreatedAt
    FROM #SampleIssues
    ORDER BY IssueID;
END
ELSE
BEGIN
    PRINT 'Sample data completed with no warnings.';
END;
